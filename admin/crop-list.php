...
$conn = mysqli_connect("localhost", "root", "root", "id17402499_bbdms");
	if (isset($_POST["delete"])) {
		$dat = $_POST['nam'];
		$record1 = mysqli_query($conn, "Delete from addcrop where filename = '$dat'") or die("error");
		if ($record1) {
			$msg = "record deleted successfully";
		} else {
									...
									<table id="data-table" class="table table-fixed table-striped table-bordered table-hover table-sm">
										<thead>
											<tr>
												...
												<th class="col-xs-2">Crop Name</th>
												<th class="col-xs-3">Climate and Soil</th>
												<th class="col-xs-3">Youtube Link</th>
												<th class="col-xs-3">Image File</th>
										...
										<tbody>
											...
											?>
												<tr>
												    <td class="col-xs-1"><?php echo $count; ?></td>
													<td class="col-xs-2"><?php echo $data['cropname']; ?></td>
													<td class="col-xs-3"><?php echo $data['climateandsoil']; ?></td>
													<td class="col-xs-3"><?php echo $data['youtube']; ?></td>
													<td class="col-xs-3"><a href="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($data['image']); ?>" download="<?php echo $data['filename']; ?>">Download<a></td>
													<td>
														<form method="post">
